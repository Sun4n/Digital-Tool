        ...
        <>
            <div className='max-w-[1200px] mx-auto text-center space-y-[16px]'>
                ...
                <div className='border inline-block border-[#F6F6F6] rounded-[1000px]'>
                    ...
                    <button onClick={() => setIsSelected(false)} className={`px-[16px] py-[12px] ${isSelected || "h-14 bg-linear-65 from-[#4F39F6] to-[#9514FA] text-white"} rounded-[1000px]`}>Cart(2)</button>
                </div>


            <div className='grid grid-cols-3 gap-[30px]'>
                {productsData.map((product, indx) => <ProductCard key={indx} product={product}></ProductCard>)}
            </div>
            </div>
        </>
    );
